...
export default function Cart() {
  ...
  const handleCheckOut = async () => {
    ...
    let response = await fetch("https://foodiebackend-bcu3.onrender.com/api/orderdata", {
      // credentials: 'include',
          ...
          <tbody>
            {data.map((food, index) => (
              <tr>
                ...
                <td>{food.size}</td>
                <td>{food.price}</td>
                <td >  <FontAwesomeIcon icon={faTrashCan} onClick={() => { dispatch({ type: "REMOVE", index: index }) }} />   </td>
